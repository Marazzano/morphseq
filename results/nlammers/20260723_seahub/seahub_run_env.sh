...
seahub_configure_run() {
    ...
    SEAHUB_GPU_CONCURRENCY="${SEAHUB_GPU_CONCURRENCY:-1}"
    if [[ ! "${SEAHUB_GPU_CONCURRENCY}" =~ ^[1-9][0-9]*$ ]]; then
        echo "ERROR: SEAHUB_GPU_CONCURRENCY must be a positive integer." >&2
        return 2
    fi
    ...
    export SEAHUB_GPU_CONCURRENCY
    SEAHUB_QSUB_ENV="SEAHUB_RUN_ID=${SEAHUB_RUN_ID},SEAHUB_OPERATIONAL_DATE=${SEAHUB_OPERATIONAL_DATE},SEAHUB_RUN_ROOT=${SEAHUB_RUN_ROOT},SEAHUB_BUNDLE_ROOT=${SEAHUB_BUNDLE_ROOT},SEAHUB_GPU_CONCURRENCY=${SEAHUB_GPU_CONCURRENCY}"
