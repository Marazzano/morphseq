"""
NWDB talk: genotype classification plots (no null) — overlay Het vs WT and Homo vs WT.

Generates one overlay figure per feature set.
"""
    ...
    for key, spec in feature_sets.items():
        ...
        for with_sig in (False, True):
            ...
            plot_multiple_aurocs(
                ...
                colors_dict=colors,
                title=f"{spec['label']} — Het/Homo vs WT (AUROC)" + (f" (p ≤ {sig_threshold:g})" if with_sig else ""),
                ax=ax,
                ylim=(0.3, 1.05),
            ...
            for outside, rect, close in (
                ...
                out_png = fig_dir / f"cep290_ref_{key}_het_and_homo_vs_wt_{suffix}{extra}.png"
                out_pdf = fig_dir / f"cep290_ref_{key}_het_and_homo_vs_wt_{suffix}{extra}.pdf"
