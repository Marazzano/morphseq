"""
NWDB backup: genotype classification overlay (with null + significance) — Het vs WT and Homo vs WT.

Generates one overlay figure per feature set with permutation null bands and significance markers.
"""
...
def main() -> None:
    ...
    for key, spec in feature_sets.items():
        ...
        auroc_dfs: dict[str, object] = {}
        colors: dict[str, str] = {}
        for (posneg, label, color) in curves:
            auroc_dfs[label] = res[posneg].copy()
            colors[label] = str(color)

        plot_multiple_aurocs(
            auroc_dfs_dict=auroc_dfs,  # type: ignore[arg-type]
            colors_dict=colors,
            title=f"{spec['label']} — Het/Homo vs WT (AUROC, null bands; p ≤ {sig_threshold:g})",
            ax=ax,
            ylim=(0.3, 1.05),
        ...
        for outside, rect, close in (
            ...
            out_png = fig_dir / f"cep290_ref_{key}_het_and_homo_vs_wt_with_null{extra}.png"
            out_pdf = fig_dir / f"cep290_ref_{key}_het_and_homo_vs_wt_with_null{extra}.pdf"
