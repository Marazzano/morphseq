...
# ---------------------------------------------------------------------------
# Experimental statistic: HDR mass-area concentration
# ---------------------------------------------------------------------------

DEFAULT_HDR_MASS_LEVELS = np.round(np.arange(0.1, 1.0, 0.1), 2)


def hdr_mass_area_profile(
    points_2d: np.ndarray,
    grid_size: int = GRID_SIZE,
    mass_levels: np.ndarray | list[float] | tuple[float, ...] = DEFAULT_HDR_MASS_LEVELS,
    *,
    kde: KDESpec | Callable | None = None,
) -> dict:
    """Highest-density-region area curve for an already-normalized 2-D cloud.

    For each requested mass fraction m, this sorts KDE grid cells by density and
    returns the grid area needed to capture the densest m fraction of KDE mass.
    Lower area means probability mass is more concentrated. Component counts are
    intentionally not part of this primary statistic.
    """
    xx, yy, density = _kde_grid(points_2d, grid_size=grid_size, kde=kde)
    mass_levels = np.asarray(mass_levels, dtype=float)
    if mass_levels.ndim != 1:
        raise ValueError("mass_levels must be one-dimensional")
    if np.any((mass_levels <= 0) | (mass_levels > 1)):
        raise ValueError("mass_levels must be in (0, 1]")

    dx = float(np.mean(np.diff(xx[0, :]))) if xx.shape[1] > 1 else 1.0
    dy = float(np.mean(np.diff(yy[:, 0]))) if yy.shape[0] > 1 else 1.0
    cell_area = abs(dx * dy)
    flat_density = np.asarray(density, dtype=float).ravel()
    total_density = float(flat_density.sum())
    if total_density <= 0 or cell_area <= 0:
        return {
            "mass_levels": mass_levels,
            "areas": np.full_like(mass_levels, np.nan, dtype=float),
            "relative_areas": np.full_like(mass_levels, np.nan, dtype=float),
            "thresholds": np.full_like(mass_levels, np.nan, dtype=float),
        }

    order = np.argsort(flat_density)[::-1]
    sorted_density = flat_density[order]
    ...
    for mass in mass_levels:
        idx = int(np.searchsorted(cumulative_mass, mass, side="left"))
        idx = min(idx, len(sorted_density) - 1)
        areas.append((idx + 1) * cell_area)
        thresholds.append(sorted_density[idx])

    areas = np.asarray(areas, dtype=float)
    denom_idx = int(np.argmin(np.abs(mass_levels - 0.9)))
    denom = float(areas[denom_idx])
    relative = areas / denom if denom > 0 else np.full_like(areas, np.nan)
    return {
        "mass_levels": mass_levels,
        "areas": areas,
        "relative_areas": relative,
...
def hdr_area_auc(
    ...
    *,
    ...
    """AUC of the HDR mass-area curve. Lower values mean more concentrated mass."""
    profile = hdr_mass_area_profile(
        points_2d,
        grid_size=grid_size,
        mass_levels=mass_levels,
        kde=kde,
    )
    key = "relative_areas" if relative else "areas"
    return float(np.trapz(profile[key], profile["mass_levels"]))


def hdr_concentration_auc(
    points_2d: np.ndarray,
    grid_size: int = GRID_SIZE,
    mass_levels: np.ndarray | list[float] | tuple[float, ...] = DEFAULT_HDR_MASS_LEVELS,
    *,
    ...
    """Concentration statistic with larger values meaning more concentrated mass."""
    return -hdr_area_auc(
        points_2d,
        grid_size=grid_size,
        mass_levels=mass_levels,
...
def critical_major_connection_ratio(
    points_2d: np.ndarray,
    k: int = 3,
    min_component_mass: float = 0.10,
    covered_mass: float = 0.85,
) -> float:
    """Alpha-like connection scale for mass-significant components only.

    Sweeps Delaunay edge lengths as in `critical_connection_ratio`, but counts only
    components holding at least `min_component_mass` of points. The statistic is the
    first edge length where all covered mass-significant components have merged into
    <=1 component, after the major components collectively cover at least
    `covered_mass` of the sample.

    This explicitly separates "small outlier components" from "multiple large
    support regions", which the largest-component criterion cannot do.
    """
    pts = np.asarray(points_2d, dtype=float)
    n = len(pts)
    if n < 2:
        return 0.0
    min_count = max(1, int(np.ceil(float(min_component_mass) * n)))
    covered_count = max(min_count, int(np.ceil(float(covered_mass) * n)))
    ...
    def major_state():
        ...
        root_ids, counts = np.unique(roots, return_counts=True)
        major = counts[counts >= min_count]
        return int(len(major)), int(major.sum()) if major.size else 0
    ...
    for length, edge in zip(edge_lengths, edges):
        ...
        if ri != rj:
            ...
            parent[rj] = ri
            size[ri] += size[rj]
        n_major, covered = major_state()
        if covered >= covered_count and n_major <= 1:
            return float(length / r_local)
    return float(edge_lengths[-1] / r_local)
...
SUPPORT_STATISTICS = {
    ...
    # HDR mass-area concentration: a DENSITY witness that runs in parallel with
    # valley_depth. valley_depth asks "is there an empty gap BETWEEN modes" (fragile
    # once the KDE bandwidth oversmooths a thin/sparse middle -- see STATUS §6); this
    # asks the complementary, sweep-free question "does the target's mass concentrate
    # into LESS KDE area than a matched-N WT null." It catches asymmetric/unequal
    # splits (three_discrete / small_middle) that valley steps over, without needing
    # the super-level component sweep to resolve a clean valley floor. Larger value =>
    # more concentrated => more discrete, so it fits the shared `null >= obs` p-value
    # convention directly. Reported as its own StatResult, NOT fused with valley.
    "hdr_area_concentration": lambda pts: hdr_concentration_auc(pts, relative=True),
...
STATISTIC_MIN_N = {
    "valley_depth": 10,
    "hdr_area_concentration": 10,  # KDE-grid based, same estimability floor as valley
    "mst_max_edge": 5,
    "fiedler": 8,
    "conductance": 8,
}
...
def compute_support_geometry(
    ...
    if kde is not None and "hdr_area_concentration" in statistics:
        statistics["hdr_area_concentration"] = (
            lambda pts: hdr_concentration_auc(pts, relative=True, kde=kde)
        )
