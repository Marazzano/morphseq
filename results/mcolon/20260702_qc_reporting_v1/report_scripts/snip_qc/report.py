"""snip_qc report — exclusion reasons over time, dead vs not-dead. TIER-1 / named ask.
...
def build(output_dir: Path) -> list[Path]:
    qc = merged("quality_control", "snip_qc", "snip_qc", ext="parquet")
    qc = qc.copy()
    ...
    combined = plot_labeled_series_over_time(
        ...
        output_path=output_dir / "exclusion_reasons_over_time.png",
        panels=[
