...
We don't care about losing dead embryos to a stricter k_lower -- the question is whether
tightening k_lower would catch real, living, small embryos in its net. Filters to
death_detection viability_dead_flag==False AND persistence_dead_flag==False before ranking.
"""
...
out = border.copy()
out.to_csv("results/mcolon/20260702_qc_reporting_v1/scratch_scripts/lower_border_snips_alive_only.csv", index=False)
...
gallery = _render_band_page(
    ...
    title="surface_area_qc (ALIVE ONLY) -- snips nearest the LOWER band threshold (k_lower=0.7), area in mm^2",
    output_path=Path("results/mcolon/20260702_qc_reporting_v1/scratch_scripts/lower_border_gallery_alive_only.png"),
    n_per_band=16,
    band_cols=4,
)
print("saved", gallery)
