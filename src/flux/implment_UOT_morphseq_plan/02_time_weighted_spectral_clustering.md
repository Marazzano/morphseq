# Task 02 — `time_weighted_spectral_clustering`

**File:** `src/flux/sampling/spectral.py`

## Objective
Group samples into clusters that are **morphologically similar** while staying **temporally adjacent** to support stable teacher‑student pairing during UOT.

## Interface (proposed)
```python
def time_weighted_spectral_clustering(
    X,                 # (N, d) embeddings
    times,             # (N,) scalar times
    n_clusters: int,
    ...
    """Return cluster labels (N,) and affinity diagnostics."""
```

## Core Steps
1. Build joint affinity `A = A_space ⊙ A_time`.
   - `A_space`: RBF on feature space (optionally KNN‑sparsified)
   - `A_time`: RBF on |t_i − t_j|
2. Normalize Laplacian; run eigen‑decomposition; apply K‑means in eigen‑space.
3. Return `labels` and optional `A` diagnostics.
4. I think we should sparsify by defaults for spectral clsutering (think about a snippet I can add to help the agent omplmemt this) as tis has minimal prrfomance loss

## Success Criteria
- Visualize t‑SNE/UMAP with colors by `labels` and by `times`; clusters should be time‑coherent.
- Quantify temporal adjacency: mean within‑cluster |Δt| is small.
