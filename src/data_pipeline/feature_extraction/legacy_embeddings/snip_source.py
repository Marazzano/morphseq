...
- ``processed_snip_path`` — the model-input PNG, stored **relative to `output_root`**
  (``data_pipeline_output``), matching the write-side contract in
  ``snip_processing/io.py::rel_to_root`` (resolved here against ``output_root``, not
...
def collect_snip_inputs(
    ...
    *,
    ...
    ``processed_snip_path`` is stored relative to ``output_root``; it is resolved here
    so callers get absolute, existence-checkable paths.
    ...
    Args:
        ...
        output_root: the pipeline output root (``data_pipeline_output``) that
            ``processed_snip_path`` is stored relative to (matches the write-side
            contract in ``snip_processing/io.py::rel_to_root``).
    ...
    resolved = resolve_snip_inventory_image_paths(df, output_root=root)
    inputs: List[SnipInput] = []
    for snip_id, image_path in zip(resolved["snip_id"], resolved["resolved_image_path"]):
        path = Path(str(image_path))
        if not path.exists():
            raise FileNotFoundError(
                f"Processed snip image for {snip_id} not found: {path} "
                f"(from processed_snip_path resolved against output_root={root}). "
                f"The snip_inventory is stale or the snips were not materialized."
            )
        inputs.append(SnipInput(snip_id=str(snip_id), image_path=path))
