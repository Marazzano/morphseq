...
rule map_positions_to_wells_yx1:
    input:
        scope_csv=EXPERIMENT_METADATA_DIR / "{experiment}" / "scope" / "yx1" / "scope_series_metadata_raw.csv",
        raw_images_dir=lambda wc: RAW_IMAGES_DIR / "YX1" / wc.experiment
    output:
        mapping_csv=EXPERIMENT_METADATA_DIR / "{experiment}" / "scope" / "yx1" / "position_well_mapping.csv",
    ...
    shell:
        (
            'PYTHONPATH="{params.pythonpath}" "{params.python}" -m data_pipeline.pipeline_orchestrator.tasks '
            'map-positions-to-wells --experiment "{params.experiment}" --microscope "YX1" '
...
rule map_positions_to_wells_keyence:
    input:
        scope_csv=EXPERIMENT_METADATA_DIR / "{experiment}" / "scope" / "keyence" / "scope_series_metadata_raw.csv",
        raw_images_dir=lambda wc: RAW_IMAGES_DIR / "Keyence" / wc.experiment
    output:
        mapping_csv=EXPERIMENT_METADATA_DIR / "{experiment}" / "scope" / "keyence" / "position_well_mapping.csv",
    ...
    shell:
        (
            'PYTHONPATH="{params.pythonpath}" "{params.python}" -m data_pipeline.pipeline_orchestrator.tasks '
            'map-positions-to-wells --experiment "{params.experiment}" --microscope "Keyence" '
            '--scope-csv "{input.scope_csv}" '
            '--raw-images-dir "{input.raw_images_dir}" --output-mapping-csv "{output.mapping_csv}" '
...
rule apply_position_to_well_mapping_yx1:
    input:
        scope_csv=EXPERIMENT_METADATA_DIR / "{experiment}" / "scope" / "yx1" / "scope_series_metadata_raw.csv",
        mapping_csv=EXPERIMENT_METADATA_DIR / "{experiment}" / "scope" / "yx1" / "position_well_mapping.csv",
    output:
    ...
    shell:
        (
            'PYTHONPATH="{params.pythonpath}" "{params.python}" -m data_pipeline.pipeline_orchestrator.tasks '
            'apply-position-to-well-mapping --experiment "{params.experiment}" --scope-csv "{input.scope_csv}" '
            '--mapping-csv "{input.mapping_csv}" --output-csv "{output.mapped_scope_csv}" '
            '--selected-wells "{params.selected_wells}"'
        )


rule apply_position_to_well_mapping_keyence:
    input:
        scope_csv=EXPERIMENT_METADATA_DIR / "{experiment}" / "scope" / "keyence" / "scope_series_metadata_raw.csv",
        mapping_csv=EXPERIMENT_METADATA_DIR / "{experiment}" / "scope" / "keyence" / "position_well_mapping.csv",
    output:
...
rule validate_physical_well_mapping_yx1:
    """Validate that physical position-to-well mapping is complete and canonical."""
    input:
        scope_csv=EXPERIMENT_METADATA_DIR / "{experiment}" / "scope" / "yx1" / "scope_series_metadata_raw.csv",
        mapping_csv=EXPERIMENT_METADATA_DIR / "{experiment}" / "scope" / "yx1" / "position_well_mapping.csv",
    output:
