...
NOTE: the MVP consolidates mask_geometry + curvature_metrics + pose_kinematics + stage_predictions.
fraction_alive is intentionally omitted until its VIA/embryo mask resolution mismatch is resolved.
"""

import importlib.util

_cf_paths_spec = importlib.util.spec_from_file_location(
    "_pipeline_orchestrator_paths_consolidated_features",
    PROJECT_ROOT / "src" / "data_pipeline" / "pipeline_orchestrator" / "orchestration" / "paths.py",
)
_cf_paths = importlib.util.module_from_spec(_cf_paths_spec)
_cf_paths_spec.loader.exec_module(_cf_paths)
...
_CF_SOURCE_STEPS = ["mask_geometry", "curvature_metrics", "pose_kinematics", "stage_predictions"]


def _cf_artifact(experiment, *, path_mode, well_id=None):
    return _cf_paths.artifact_path(
        DATA_ROOT, CONSOLIDATED_FEATURES_STEP, "consolidated_features", experiment,
        path_mode=path_mode, well_id=well_id,
    )


def _cf_validated(experiment, *, path_mode, well_id=None):
    return _cf_paths.validated_path(
        DATA_ROOT, CONSOLIDATED_FEATURES_STEP, "consolidated_features", experiment,
        path_mode=path_mode, well_id=well_id,
    )
...
def _cf_registry(experiment, *, well_id):
    return _cf_paths.artifact_path(
        ...
        path_mode=_cf_paths.PATH_MODE_PER_WELL, well_id=well_id,
    )
...
def _cf_source_shards(experiment, well_id):
    ...
    for step in _CF_SOURCE_STEPS:
        deps.append(str(_cf_paths.artifact_path(
            DATA_ROOT, step, step, experiment, path_mode=_cf_paths.PATH_MODE_PER_WELL, well_id=well_id,
        )))
        deps.append(str(_cf_paths.validated_path(
            DATA_ROOT, step, step, experiment, path_mode=_cf_paths.PATH_MODE_PER_WELL, well_id=well_id,
        )))
    return deps
...
rule build_consolidated_features_for_well:
    ...
    output:
        consolidated=str(_cf_artifact(
            "{experiment}", path_mode=_cf_paths.PATH_MODE_PER_WELL, well_id="{well_id}"
        )),
    shell:
        """
...
rule merge_consolidated_features:
    ...
    input:
        ...
        per_well_validated=lambda wc: [
            str(_cf_validated(wc.experiment, path_mode=_cf_paths.PATH_MODE_PER_WELL, well_id=w))
            for w in wells_for_experiment(wc)
        ],
    output:
        merged=str(_cf_artifact("{experiment}", path_mode=_cf_paths.PATH_MODE_MERGED)),
    shell:
        """
        {RUN} -c "
