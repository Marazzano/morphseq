...
def run_snip_processing(
    *,
    ...
    rendering_contract = build_rendering_contract(
        ...
        background_contract={
            # PEGGED as of 2026-08-27, so this contract describes constants rather than a
            # per-job estimate. The previous record advertised source-frame sampling with a
            # frame count, a seed, and a pixel cap; none of that runs any more, and leaving
            # those keys in place would make the sidecar assert something the renderer does
            # not do. The sampling estimator survives for diagnostics only.
            "estimated_for_job": False,
            "applied_to_pixels": is_clahe_blend,
            "model": "pegged constants (not estimated from source frames)",
            "mean": float(background_mean),
            "std": float(background_std),
            "pegged_at": "snip_processing.defaults.DEFAULT_BACKGROUND_MEAN/_STD",
            "peg_rationale": (
                "per-job estimation made the background a function of upstream source "
                "intensity; measured 2026-08-27 on 20250612_30hpf_ctrl_atf6 as 61% of the "
                "legacy-vs-regenerated pixel difference"
            ),
            "noise_distribution": {
