...
def get_detector_ingestor(backend: str) -> ModuleType:
    ...
    mod_path = {
        "groundingdino": "data_pipeline.object_extraction.segmentation._archive.segmentation_and_tracking.ingestors.gdino_ingestor",
        "detectron2": "data_pipeline.object_extraction.segmentation._archive.segmentation_and_tracking.ingestors.detectron2_ingestor",
    }.get(backend)
    if not mod_path:
...
def get_tracker_ingestor(backend: str) -> ModuleType:
    backend = str(backend).strip().lower()
    mod_path = {
        "sam2": "data_pipeline.object_extraction.segmentation._archive.segmentation_and_tracking.ingestors.sam2_ingestor",
        "sam3": "data_pipeline.object_extraction.segmentation._archive.segmentation_and_tracking.ingestors.sam3_ingestor",
    }.get(backend)
    if not mod_path:
