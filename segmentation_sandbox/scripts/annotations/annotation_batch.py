...
class AnnotationBatch(UnifiedEmbryoManager):
    ...
    def __init__(self, author: str, description: str = "", metadata_ref=None):
        ...
        self.metadata_ref = metadata_ref  # Optional reference to EmbryoMetadata
        self.created = datetime.now().isoformat()
        self.data = {"embryos": {}}  # Match structure with metadata format
        self.stats = {"phenotypes": 0, "genotypes": 0, "flags": 0, "treatments": 0}
        self.config = {"default_author": author}
        self.verbose = True
    ...
    def add_phenotype(self, embryo_id: str, phenotype: str, frames: Union[str, List[str]] = "all",
        ...
        # For batch processing, we'll store the operation parameters
        # and defer validation to when batch is applied to main metadata
        ...
        if "phenotype_operations" not in self.data["embryos"][embryo_id]:
            self.data["embryos"][embryo_id]["phenotype_operations"] = []
        
        self.data["embryos"][embryo_id]["phenotype_operations"].append(operation)
    ...
    def add_flag(self, embryo_id: str, flag_type: str, level: str = "auto",
        ...
        if "flags" not in self.data["embryos"][embryo_id]:
            self.data["embryos"][embryo_id]["flags"] = []
        
        self.data["embryos"][embryo_id]["flags"].append(flag_data)
        self.stats["flags"] += 1
    ...
    def add_treatment(self, embryo_id: str, treatment_name: str, dosage: str = None,
        ...
        treatment_id = f"{treatment_name}_{len(self.data['embryos'][embryo_id].get('treatments', {})) + 1}"
        
        if "treatments" not in self.data["embryos"][embryo_id]:
            self.data["embryos"][embryo_id]["treatments"] = {}
        
        self.data["embryos"][embryo_id]["treatments"][treatment_id] = {
    ...
    def remove_annotation(self, embryo_id: str, annotation_type: str, **filters):
        ...
        if annotation_type == "phenotype":
            self.data["embryos"][embryo_id].pop("phenotype_operations", None)
        elif annotation_type == "genotype":
            self.data["embryos"][embryo_id].pop("genotype", None)
        elif annotation_type == "flags":
            self.data["embryos"][embryo_id].pop("flags", None)
        elif annotation_type == "treatments":
            self.data["embryos"][embryo_id].pop("treatments", None)
    ...
    def edit_annotation(self, embryo_id: str, annotation_type: str, **updates):
        ...
        if annotation_type == "genotype" and "genotype" in self.data["embryos"][embryo_id]:
            self.data["embryos"][embryo_id]["genotype"].update(updates)
    ...
    def clear_embryo(self, embryo_id: str, annotation_types: Union[str, List[str]] = "all"):
        ...
        if annotation_types == "all":
            self.data["embryos"].pop(embryo_id)
        else:
            if isinstance(annotation_types, str):
                annotation_types = [annotation_types]
            
            for ann_type in annotation_types:
                self.data["embryos"][embryo_id].pop(ann_type, None)
    ...
    def __str__(self) -> str:
        ...
        lines = [
            ...
            f"Stats: {self.stats}",
            f"Embryos: {len(self.data['embryos'])}",
            ""
        ]
        
        for embryo_id, data in self.data["embryos"].items():
            lines.append(f"📋 {embryo_id}:")
            
            if "genotype" in data and data["genotype"] is not None:
                g = data["genotype"]
