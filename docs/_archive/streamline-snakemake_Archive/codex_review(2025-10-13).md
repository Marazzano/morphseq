...
1. **Scope/plate schema requires embryo IDs before segmentation**  
   - Evidence: `docs/data_pipeline/specs/data_validation_plan.md:76-92` adds `'embryo_id'` (and frame-level fields) to `REQUIRED_COLUMNS_SCOPE_AND_PLATE_METADATA`.  
...
2. **Snip ID format oscillates between `_s####` and `_t####` across docs**  
   - Evidence: `docs/data_pipeline/specs/processing_files_pipeline_structure_and_plan.md:354-355` defines snip IDs as `{embryo_id}_s{frame:04d}`, while the same file later (line 733) and the output structure spec (`docs/data_pipeline/specs/data_ouput_strcutre.md:110-118`) require `{embryo_id}_t{frame_index}`.  
...
4. **Segmentation schema mandates `video_id`, but no plan produces it**  
   - Evidence: `docs/data_pipeline/specs/data_validation_plan.md:100-135` includes `'video_id'` in `REQUIRED_COLUMNS_SEGMENTATION_TRACKING`, yet neither the segmentation outputs in `processing_files_pipeline_structure_and_plan.md` nor the Snakemake rules mention creating or propagating that field.  
...
5. **Module paths diverge between documentation sets**  
   - Evidence: The output/module plan uses `metadata/plate_processing.py` and `metadata_mapping/align_metadata.py` (`processing_files_pipeline_structure_and_plan.md:232-244`), while the Snakemake rule draft references `metadata_ingest/plate/plate_processing.py` and `metadata_ingest/mapping/align_scope_plate.py` (`docs/data_pipeline/specs/snakemake_rules_data_flow.md:24-56`).  
...
6. **Snakemake rules cite an undefined schema constant**  
   - Evidence: `docs/data_pipeline/specs/snakemake_rules_data_flow.md:37-45` calls out `REQUIRED_COLUMNS_SERIES_MAPPING`, but `docs/data_pipeline/specs/data_validation_plan.md` never defines that symbol.  
...
7. **Cross-references point to missing or differently named docs**  
   - Evidence: `docs/data_pipeline/specs/data_ouput_strcutre.md:5-158` claims alignment with `preliminary_rules.md`, but the only rule doc present is `snakemake_rules_data_flow.md`.  
...
8. **Refactoring plan placeholder is empty**  
   - Evidence: `docs/data_pipeline/specs/refactoring_plan.md` has zero lines (`wc -l` reports 0).  
