...
Snakemake rule originally declared this as one job per well. As of 2026-07-29 it is wired as
one run-level batch, so production now pays import+load once per experiment.
...
| snip_auxiliary_masks (4x UNet) | **SERVE — confirmed by a real served-vs-per-well run** | Load 69.94s paid once vs. 2.3–12.6s of actual per-well work. 3-well A/B: 280.39s per-well → 97.47s served (**2.88×**), with all 2,100 mask PNGs byte-identical. The `<50ms` figure in this row was too low — real work is seconds, not milliseconds — but the conclusion holds: load dominates by ~10×. Extrapolated over 576 wells, ~11h of repeated loading becomes ~70s. |
| latent_embeddings (VAE) | **BATCHED (2026-07-29)** | One Python-3.9 process now loads once and writes all canonical per-well shards; no resident server or GPU allocation is needed. |
